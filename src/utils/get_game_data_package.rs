...
pub async fn get_game_data_package(
    ...
    let game_data_result = client.post(base_url.clone() + &"/_api/game-data-get")
    .json(&map)
    .send()
    .await;
    ...
    // block_on(game_data);
    println!("Completed downloading game data from {}, decoding json (strlen {})", &base_url, game_data.len());
